...
const source = await readFile(resolve(process.cwd(), "contracts/TradeASkill.sol"), "utf8");
const input = {
  language: "Solidity",
  sources: { "TradeASkill.sol": { content: source } },
  settings: { outputSelection: { "*": { "*": ["abi", "evm.bytecode.object"] } } },
};
const output = JSON.parse(solc.compile(JSON.stringify(input)));
const contract = output.contracts?.["TradeASkill.sol"]?.TradeASkill;
if (!contract) {
...
console.log(`Deploying TradeASkill from ${account.address} on Base...`);
...
console.log("Set NEXT_PUBLIC_TRADE_A_SKILL_CONTRACT_ADDRESS to this contract address in Vercel.");
